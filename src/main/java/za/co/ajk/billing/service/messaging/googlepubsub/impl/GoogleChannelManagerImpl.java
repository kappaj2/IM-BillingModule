package za.co.ajk.billing.service.messaging.googlepubsub.impl;
...
import za.co.ajk.billing.config.MessageImplementationCondition;
import za.co.ajk.billing.config.PubSubMessagingProperties;
import za.co.ajk.billing.enums.PubSubMessageType;
import za.co.ajk.billing.service.messaging.IMMessageProcessor;
import za.co.ajk.billing.service.messaging.InterModulePubSubMessage;
import za.co.ajk.billing.service.messaging.googlepubsub.GoogleChannelManager;
...
public class GoogleChannelManagerImpl implements GoogleChannelManager {
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterGenericSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "BillingGenericSub");
            adapter
                .setOutputChannel(
    ...
    public PubSubInboundChannelAdapter messageChannelAdapterBillingSub(
        ...
        try {
            PubSubInboundChannelAdapter adapter =
                new PubSubInboundChannelAdapter(pubSubTemplate, "BillingTopicSub");
            adapter
                .setOutputChannel(
                    inputChannel); // looks like the channel to ack on (thus the input channel - confusing!)
            adapter.setAckMode(AckMode.MANUAL);
